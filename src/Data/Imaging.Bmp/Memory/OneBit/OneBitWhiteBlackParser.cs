...
/// <summary>
/// A <see cref="IMemoryParser"/> for 1bit bitmaps where zero bits are interpreted as white.
/// </summary>
public sealed class OneBitWhiteBlackParser : IMemoryParser
{
    #region Dependencies

    /// <summary>
    /// Creates a new instance of the <see cref="OneBitWhiteBlackParser"/> type.
    /// </summary>
    ...
    public OneBitWhiteBlackParser(MemoryAlignment alignment, int height, uint width)
    {
        Alignment = alignment;
        Height = height;
        Width = width;
    ...
    public IImageMemory Parse(BinaryReader reader)
    {
        ...
        if (Height > 0) // rows are stored bottom up
        {
            for (var h = Height - 1; h >= 0; --h)
            {
                ...
                {
                    data[offset + w] = (byte)~reader.ReadByte();
                }
